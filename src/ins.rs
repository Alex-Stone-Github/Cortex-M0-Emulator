...
pub struct InsType {
    ...
    pub execute: fn(&InsData, &mut registers::Registers, &mut [AByte])
}
...
impl LoaderExecuter {
    ...
    pub fn implement(&mut self,
        ...
        execute: fn(&InsData, &mut registers::Registers, &mut [AByte])
            ) {
        let ins_type = InsType{name, is_me, execute};
        self.instruction_types.push(ins_type);
    }
    pub fn execute(&self, instruction: &InsData, regs: &mut registers::Registers, memory: &mut [AByte]) {
        ...
        println!("Executed `{}`!", instruction_type.name);
    }
}
